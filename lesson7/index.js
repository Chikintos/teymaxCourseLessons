...
Доработать функции сreate, read,readAll, update и remove, которые будет имплементировать `CRUD` модель.
...
const create = (person) => {
    i+=1
    id=i
    mapemp.set(+id,person)
    console.log(mapemp.get(id))
    return id
...
const read = (id) => {
  ...
  var ishas=mapemp.has(+id)
    if (ishas){
        return mapemp.get(+id)
    }
    else {
        document.getElementById('alert').innerText="Робiтника з даним id не знайдено"
        return null
    }
...
const update = (uid, { name, salary }) => {
    const oldinfo =mapemp.get(+uid)
    oldinfo.name=name
    oldinfo.salary=salary
    mapemp.delete(+uid)
    mapemp.set(+uid,oldinfo)

}
const remove = (id) => {
    mapemp.delete(+id)
    return !mapemp.has(+id)
